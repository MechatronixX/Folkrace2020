...
Servo::Servo()
{
    ...
    for (int i = LEDC_CHANNEL_OFFSET; i < MAX_SERVOS + 1; i++)
    {
        if (ChannelUsed[i] == -1)
        {
...
int Servo::attach(int pin, int min, int max)
{
    if ((this->servoChannel <= MAX_SERVOS) && (this->servoChannel > 0))
    {
        ...
        if (this->pinNumber < 0)
        { // we are attaching to a new or previously detached pin; we need to initialize/reinitialize
        ...
        // int servo_channel_debug = 3;
        // ledcSetup(servo_channel_debug, REFRESH_CPS, this->timer_width); // channel #, 50 Hz, timer width
        // ledcAttachPin(this->pinNumber, servo_channel_debug);            // GPIO pin assigned to channel
    }
    else
    {
        return 0;
